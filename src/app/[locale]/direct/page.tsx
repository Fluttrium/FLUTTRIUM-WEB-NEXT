                    ...
                    {projects.map((project, index) => (
                        ...
                        >
                            <div className="flex flex-col md:flex-row items-start justify-between mb-4 md:mb-6 gap-4">
                                <div className="flex flex-col sm:flex-row items-center gap-3 md:gap-4">
                                    ...
                                    <div className="text-center sm:text-left">
                                        <h3 className="text-lg md:text-2xl font-bold text-white mb-1">{project.title}</h3>
                                        <div className="text-sm md:text-base text-green-400 font-medium animate-pulse">✅ Успешно внедрен • {project.result}</div>
                                    </div>
                                </div>
                            ...
                            <div className="mb-4 md:mb-6">
                                <h4 className="text-white font-semibold mb-2 md:mb-3 text-base md:text-lg">Ключевые результаты:</h4>
                ...
                <div className={`text-center transform transition-all duration-1000 delay-400 ${
                    ...
                    <div className="inline-flex flex-col md:flex-row items-center gap-4 md:gap-6 p-4 md:p-8 bg-gradient-to-r from-blue-900/20 to-purple-900/20 rounded-2xl md:rounded-3xl border border-blue-500/30 backdrop-blur-sm hover:scale-105 transition-all duration-500 hover:shadow-2xl hover:shadow-blue-500/25">
                        <div className="text-center md:text-left">
                            <div className="text-xl md:text-2xl lg:text-3xl font-bold text-white mb-2 md:mb-3">{messages.DirectPage?.successCases?.cta?.title || "Нужно SEO-продвижение?"}</div>
                            <div className="text-base md:text-lg text-gray-300">{messages.DirectPage?.successCases?.cta?.subtitle || "Мы поднимем ваш сайт в ТОП Яндекса и Google, даже в самых конкурентных нишах"}</div>
                        </div>
                        <button
                            onClick={handleOpenModal}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-8">
                    {features.map((feature, index) => (
                        <div
                            ...
                            style={{ transitionDelay: `${index * 200}ms` }}
                        >
                            <div className="mb-6 md:mb-8 group-hover:scale-110 transition-transform duration-300">{feature.icon}</div>
                            <h3 className="text-xl md:text-2xl font-bold text-white mb-4 md:mb-6">{feature.title}</h3>
                            <ul className="text-base md:text-lg text-gray-300 space-y-2 md:space-y-3">
                                {feature.items.map((item, idx) => (
                                    <li key={idx} className="flex items-start gap-2 md:gap-3 hover:text-white transition-colors duration-300">
                                        <Check className="w-4 h-4 md:w-5 md:h-5 text-green-400 mt-1 flex-shrink-0 animate-pulse" style={{animationDelay: `${idx * 100}ms`}} />
                                        <span>{item}</span>
                                    </li>
                                ))}
                            </ul>
                    ...
                    {steps.map((item, index) => (
                        <div key={index} className="text-center group">
                            <div className="relative mb-6 md:mb-8">
                                <div className={`w-16 h-16 md:w-20 md:h-20 bg-gradient-to-r from-blue-500 to-purple-500 rounded-full flex items-center justify-center text-white font-bold text-xl md:text-2xl mx-auto mb-3 md:mb-4 group-hover:scale-110 ${animations.transition.normal} shadow-lg`}>
                                    {item.step}
                                </div>
