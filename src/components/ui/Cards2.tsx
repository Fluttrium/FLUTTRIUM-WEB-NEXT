  ...
  const projects = [
    {
      ...
      description: m?.benefits?.cost?.description || "Экономия бюджета, в среднем, от 30% до 50%. Вы не платите за два отдельных проекта, как при использовании нативных решений",
    },
    {
      title: m?.benefits?.speed?.title || "Скорость выше",
      description: m?.benefits?.speed?.description || "Сокращение сроков выхода на рынок. Один Flutter-разработчиков может выполнять задачи двух нативных в те же сроки",
    },
    {
      title: m?.benefits?.performance?.title || "Производительность",
      description: m?.benefits?.performance?.description || "Пользователи не заметят разницы нативом - приложения на Flutter работают быстро и эффективно",
    },
    {
      title: m?.benefits?.functionality?.title || "Функциональность",
      description: m?.benefits?.functionality?.description || "Flutter-приложения создаются для ритейла, банков, страховых и других бизнесов без ограничений",
    },
  ];
