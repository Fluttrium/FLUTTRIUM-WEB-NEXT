...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  const _heightTransform = useTransform(scrollYProgress, [0, 1], [1, height]);
  const _opacityTransform = useTransform(scrollYProgress, [0, 0.1], [0, 1]);

  return (
    <div className="m-1 font-sans md:px-10" ref={containerRef}>
      ...
      <div ref={ref} className="relative max-w-7xl mx-auto pb-10 md:pb-20">
        {data.map((item, index) => (
          ...
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-20 sm:top-24 md:top-32 lg:top-40 self-start max-w-full md:max-w-xs lg:max-w-sm md:w-full">
              <div className="h-8 sm:h-10 absolute left-2 sm:left-3 md:left-3 w-8 sm:w-10 rounded-full bg-white dark:bg-black flex items-center justify-center">
                <div className="h-3 w-3 sm:h-4 sm:w-4 rounded-full bg-neutral-200 dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-700 p-1.5 sm:p-2" />
              </div>
            ...
            <div className="relative pl-16 sm:pl-20 pr-2 sm:pr-4 md:pl-4 w-full">
              ...
              <div className="max-w-full overflow-hidden">{item.content}</div>
            </div>
          </div>
        ))}
