    ...
    <section className="w-full z-50 overflow-hidden">
      ...
      <div className="px-4 sm:px-6 md:px-10 my-7">
        ...
        <div className="sm:hidden">
          <div
            className={`flex space-x-6 ${!isReducedMotion ? "animate-scroll-projects" : ""}`}
          >
            {/* Дублируем карточки для бесконечной прокрутки */}
            {[...projects, ...projects].map(
              ({ title, description, imgSrc }, index) => (
                <CardContainer
                  key={`mobile-${index}`}
                  className="inter-var mb-6 w-64 h-80 flex-shrink-0"
                >
        ...
        <div className="hidden sm:grid lg:hidden grid-cols-2 gap-4 md:gap-6">
          {projects.map(({ title, description, imgSrc }, index) => (
            <CardContainer
              key={`tablet-${index}`}
              className="inter-var mb-6 w-full h-max"
            >
              <CardBody className="bg-gray-50 relative group/card dark:hover:shadow-2xl dark:hover:shadow-emerald-500/[0.1] dark:bg-black dark:border-white/[0.2] border-black/[0.1] w-full h-full rounded-xl p-4 border">
                <CardItem
                  translateZ="50"
